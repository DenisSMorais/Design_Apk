...
public class DesignView extends View {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        // Draw current figure being drawn or resized
    ...
    private void resizeCurrentFigure(float x, float y) {
        if (currentFigure != null) {
            List<PointF> points = currentFigure.getPoints();
            if (points.size() > 0) {
                PointF lastPoint = points.get(points.size() - 1);
                float deltaX = x - lastPoint.x;
                float deltaY = y - lastPoint.y;
